...
// Find ALL open slots of given duration in a range (used to generate alternates for deny/retry)
...
// Look at past approved scheduling feedback for tasks with overlapping keywords; return preferred start-minute-of-day (mode), or null.
...
// Build a ranked list of full occurrence-sets (each set sums exactly to estimatedMinutes). First = learned/best guess.
function suggestOccurrenceCandidates(task, allData, chunkMinutes, maxCandidates) {
  ...
  // Build per-day candidate start options (ranked: learned time first, then other open slots)
...
// ---- Smart scheduler: chunk-size prompt -> learned/ranked candidate -> approve or deny-and-retry ----
function TaskModal({ task, onSave, onClose, defaultCategory, goals, allData, jumpToScheduler, onRecordFeedback }) {
  ...
  const [schedPhase, setSchedPhase] = useState(null); // null | "askChunk" | "reviewing"
...
// ---- Mobile-friendly task card (used in narrow layout instead of the table) ----
