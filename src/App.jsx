...
const STOPWORDS = new Set(["the", "a", "an", "to", "for", "of", "and", "or", "my", "this", "that", "with", "on", "in", "at", "is", "it"]);
const keywordsOf = (title) => (title || "").toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(w => w.length > 2 && !STOPWORDS.has(w));
...
const GOAL_TEMPLATE_TASKS = { fitness: ["Buy/check needed equipment", "Schedule first week of workouts", "Take starting photo/measurements", "Plan grocery list for the week"], work: ["Break this into 3 milestones", "Block focus time on calendar", "Identify first blocker to resolve"], personal: ["Define what 'done' looks like", "List first 3 concrete actions", "Set a check-in reminder in 2 weeks"] };
...
function emptyData() {
  return { settings: DEFAULT_SETTINGS, tasks: [], goals: [], habits: [], events: [], dailyLogs: {}, completedLog: [], workoutLog: [], bodyStats: [], exerciseHistory: [], currentPlan: null, theme: "dark", inbox: [], scheduleTemplates: [], weeklyReviews: [], schedulingFeedback: [] };
}
...
function personalWindowsForDay(settings, dateStr) { const dow = dowOf(dateStr); return (settings.personalWindows || DEFAULT_PERSONAL_WINDOWS).filter(w => w.days.includes(dow)); }
function lowEnergyRangesForDay(settings, dateStr) { const dow = dowOf(dateStr); return (settings.lowEnergyWindows || []).filter(w => w.days.includes(dow)).map(w => ({ start: timeToMinutes(w.start), end: timeToMinutes(w.end) })); }
function recurringTaskAppliesOn(task, dateStr) { if (task.taskType !== "recurring") return false; const dow = dowOf(dateStr); if (task.recurFrequency === "daily") return true; if (task.recurFrequency === "weekly") return (task.recurDays || []).includes(dow); return false; }
function eventOccursOn(event, dateStr) { if (event.date === dateStr) return true; if (!event.recurring) return false; if (dateStr < event.date) return false; if (event.recurEnd && dateStr > event.recurEnd) return false; const dow = dowOf(dateStr), startDow = dowOf(event.date); if (event.recurFrequency === "weekly") return dow === startDow; if (event.recurFrequency === "daily") return true; return false; }
...
function findAllOpenSlots(duration, rangeStart, rangeEnd, occupied, avoid, maxResults) {
  const blockers = avoid ? [...occupied, ...avoid] : occupied;
  const sorted = [...blockers].sort((a, b) => a.start - b.start);
  const results = [];
  let cursor = rangeStart;
  for (const slot of sorted) {
    if (slot.start - cursor >= duration) results.push(cursor);
    cursor = Math.max(cursor, slot.end);
    if (results.length >= maxResults) return results;
  }
  if (rangeEnd - cursor >= duration) results.push(cursor);
...
function learnedPreferredTime(taskTitle, schedulingFeedback) {
  const kws = keywordsOf(taskTitle);
  if (kws.length === 0 || !schedulingFeedback || schedulingFeedback.length === 0) return null;
  const matches = schedulingFeedback.filter(f => f.approved && keywordsOf(f.title).some(k => kws.includes(k)));
  if (matches.length === 0) return null;
  const buckets = {};
  matches.forEach(m => { const bucket = Math.round(m.start / 30) * 30; buckets[bucket] = (buckets[bucket] || 0) + 1; });
  const best = Object.entries(buckets).sort((a, b) => b[1] - a[1])[0];
  return best ? Number(best[0]) : null;
}

function rangeForTask(task, settings, dateStr) {
  const dow = dowOf(dateStr);
  if (task.category === "work") { if (!(settings.workDays || [1, 2, 3, 4, 5]).includes(dow)) return null; return [timeToMinutes(settings.workStart), timeToMinutes(settings.workEnd)]; }
  const windows = personalWindowsForDay(settings, dateStr);
  if (windows.length === 0) return [timeToMinutes(settings.wakeTime), timeToMinutes(settings.bedTime)];
  return [timeToMinutes(windows[0].start), timeToMinutes(windows[0].end)];
}

function suggestOccurrenceCandidates(task, allData, chunkMinutes, maxCandidates) {
  const { settings, schedulingFeedback } = allData;
  const totalMinutes = task.estimatedMinutes || 30;
  const today = todayStr();
  const endDate = task.targetDate || task.dueDate || addDays(today, 7);
  const days = dateRangeDays(today < endDate ? today : endDate, today < endDate ? endDate : today);
  const prioRank = { high: 0, medium: 1, low: 2 };
  const rank = prioRank[task.priority] ?? 1;
  const numChunks = Math.max(1, Math.ceil(totalMinutes / chunkMinutes));
  const spreadCount = Math.min(days.length || 1, numChunks === 1 ? 1 : (rank === 0 ? days.length : rank === 1 ? Math.ceil(days.length * 0.6) : Math.max(1, Math.ceil(days.length * 0.3))));
  const chosenDays = ((rank === 2 ? days.slice(-spreadCount) : days.slice(0, spreadCount)).slice(0, Math.max(numChunks, 1))).length ? (rank === 2 ? days.slice(-spreadCount) : days.slice(0, spreadCount)).slice(0, Math.max(numChunks, 14)) : [today];

  const learnedStart = learnedPreferredTime(task.title, schedulingFeedback);
  const avoidLowEnergy = task.avoidLowEnergy !== false;

  const perDayOptions = chosenDays.map(d => {
    const range = rangeForTask(task, settings, d);
    if (!range) return { date: d, options: [] };
    const occupied = getFixedOccupiedForDay(d, allData);
    const avoid = avoidLowEnergy ? lowEnergyRangesForDay(settings, d) : [];
    const isLastDay = d === chosenDays[chosenDays.length - 1];
    const thisChunk = isLastDay ? (totalMinutes - chunkMinutes * (chosenDays.length - 1)) : chunkMinutes;
    const duration = Math.max(15, thisChunk);
    let options = findAllOpenSlots(duration, range[0], range[1], occupied, avoid, 5);
    if (options.length === 0) options = findAllOpenSlots(duration, timeToMinutes(settings.wakeTime), timeToMinutes(settings.bedTime), occupied, avoid, 5);
    if (options.length === 0) options = findAllOpenSlots(duration, timeToMinutes(settings.wakeTime), timeToMinutes(settings.bedTime), occupied, [], 5);
    if (learnedStart !== null) options.sort((a, b) => Math.abs(a - learnedStart) - Math.abs(b - learnedStart));
    return { date: d, duration, options };
  });
  ...
  for (let variant = 0; variant < maxCandidates; variant++) {
    const occs = [];
    let ok = true;
    perDayOptions.forEach(p => { const opt = p.options[Math.min(variant, p.options.length - 1)]; if (opt === undefined) { ok = false; return; } occs.push({ id: uid(), date: p.date, start: opt, end: opt + p.duration }); });
    if (ok && occs.length > 0) candidates.push(occs);
  }
...
function generateWeeklyReview(data, weekEndDate) {
  ...
  const habitRates = habits.filter(h => h.active).map(h => { const applicable = days.filter(d => { const dow = dowOf(d); return h.frequency === "daily" || (h.frequency === "weekly" && h.days.includes(dow)); }); const completed = applicable.filter(d => completedLog.some(c => c.habitId === h.id && c.date === d)); return { name: h.name, rate: applicable.length ? Math.round((completed.length / applicable.length) * 100) : 0 }; });
...
function ConfirmModal({ title, message, confirmLabel, onConfirm, onCancel }) {
  return (
    <Modal title={title} onClose={onCancel}>
      ...
      <div style={{ display: "flex", gap: 8 }}><button className="btn btn-primary" style={{ flex: 1, justifyContent: "center" }} onClick={onConfirm}>{confirmLabel}</button><button className="btn" style={{ flex: 1, justifyContent: "center" }} onClick={onCancel}>Cancel</button></div>
    </Modal>
  );
}
...
function OnboardingModal({ settings, onSave, onClose }) {
  ...
  return (
    <Modal title="Schedule settings" onClose={onClose} wide>
      ...
      <Field label="Personal time windows"><p style={{ fontSize: 12, color: "var(--text-tertiary)", margin: "0 0 8px" }}>Add as many separate blocks as you need.</p>{s.personalWindows.map(w => <PersonalWindowRow key={w.id} w={w} onChange={(nw) => updateWindow(w.id, nw)} onDelete={() => deleteWindow(w.id)} />)}<button className="btn btn-sm" onClick={addWindow} style={{ marginTop: 6 }}><Icon name="plus" size={13} /> Add personal time window</button></Field>
      <Field label="Low-energy windows (optional)"><p style={{ fontSize: 12, color: "var(--text-tertiary)", margin: "0 0 8px" }}>Deep-focus tasks try to avoid these hours when possible.</p>{s.lowEnergyWindows.map(w => <PersonalWindowRow key={w.id} w={w} onChange={(nw) => updateLE(w.id, nw)} onDelete={() => deleteLE(w.id)} />)}<button className="btn btn-sm" onClick={addLE} style={{ marginTop: 6 }}><Icon name="battery-low" size={13} /> Add low-energy window</button></Field>
      ...
      <div className="field-row"><Field label="Workouts per week"><input type="number" min="0" max="7" value={s.fitnessFrequency} onChange={e => setS({ ...s, fitnessFrequency: Number(e.target.value) })} /></Field><Field label="Equipment available"><select value={s.equipment} onChange={e => setS({ ...s, equipment: e.target.value })}><option value="gym">Full gym</option><option value="home">Home equipment</option><option value="minimal">Minimal / bodyweight</option></select></Field></div>
...
function EventModal({ event, onSave, onClose, defaultDate }) {
  ...
  return (
    <Modal title={event ? "Edit event" : "New meeting / appointment"} onClose={onClose}>
      ...
      <Field label="Recurrence"><label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, fontWeight: 600, marginBottom: e.recurring ? 10 : 0 }}><input type="checkbox" checked={e.recurring} onChange={ev => setE({ ...e, recurring: ev.target.checked })} style={{ width: 17, height: 17 }} /> This repeats weekly</label>{e.recurring && <div className="field-row" style={{ marginTop: 8 }}><div><label className="sublabel">Ends on (optional)</label><input type="date" value={e.recurEnd} onChange={ev => setE({ ...e, recurEnd: ev.target.value })} /></div><div style={{ display: "flex", alignItems: "flex-end" }}><p style={{ fontSize: 12, color: "var(--text-tertiary)", margin: "0 0 9px" }}>Leave blank for open-ended.</p></div></div>}</Field>
...
function TaskModal({ task, onSave, onClose, defaultCategory, goals, allData, jumpToScheduler, onRecordFeedback }) {
  ...
  const [schedPhase, setSchedPhase] = useState(null);
  const [chunkChoice, setChunkChoice] = useState("single");
  ...
  const startSuggestFlow = () => { if (t.estimatedMinutes > 45) setSchedPhase("askChunk"); else { setChunkChoice("single"); generateCandidates(t.estimatedMinutes); } };
  const generateCandidates = (chunkMinutes) => { const cands = suggestOccurrenceCandidates(t, allData, chunkMinutes, 5); setCandidates(cands); setCandidateIdx(0); setSchedPhase("reviewing"); };
  const chooseChunk = (choice) => { setChunkChoice(choice); const chunk = choice === "single" ? t.estimatedMinutes : choice === "half" ? Math.ceil(t.estimatedMinutes / 2) : Math.ceil(t.estimatedMinutes / 3); generateCandidates(chunk); };

  const currentCandidate = candidates[candidateIdx] || [];
  const approveCandidate = () => {
    setT({ ...t, occurrences: currentCandidate });
    if (onRecordFeedback) currentCandidate.forEach(o => onRecordFeedback({ title: t.title, start: o.start, approved: true, timestamp: new Date().toISOString() }));
    setSchedPhase(null);
  };
  const denyCandidate = () => {
    if (onRecordFeedback) currentCandidate.forEach(o => onRecordFeedback({ title: t.title, start: o.start, approved: false, timestamp: new Date().toISOString() }));
    if (candidateIdx + 1 < candidates.length) setCandidateIdx(candidateIdx + 1);
    else { setSchedPhase(null); setCandidates([]); }
  };

  useEffect(() => { if (jumpToScheduler) startSuggestFlow(); }, []);
      ...
      {t.taskType === "one_time" && (
        <div className="occ-section">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <button className="btn btn-sm" onClick={startSuggestFlow}><Icon name="wand" size={13} /> Suggest schedule</button>
          </div>

          {schedPhase === "askChunk" && (
            <div className="suggestion-box">
              <p style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 8 }}>This task is {t.estimatedMinutes} minutes — how would you like to chunk it?</p>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button className="btn btn-sm" onClick={() => chooseChunk("single")}>One sitting ({t.estimatedMinutes} min)</button>
                <button className="btn btn-sm" onClick={() => chooseChunk("half")}>Split in half (~{Math.ceil(t.estimatedMinutes / 2)} min each)</button>
                <button className="btn btn-sm" onClick={() => chooseChunk("third")}>Split into 3 (~{Math.ceil(t.estimatedMinutes / 3)} min each)</button>
              </div>
            </div>
          )}

          {schedPhase === "reviewing" && (
            <div className="suggestion-box">
              {currentCandidate.length === 0 ? (
                <p style={{ fontSize: 12.5, color: "var(--text-tertiary)" }}>No more open slots found — try a longer target date, shorter estimate, or add a block manually below.</p>
              ) : (
                <>
                  <p style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 6 }}>
                    Suggestion {candidateIdx + 1} of {candidates.length}{candidateIdx === 0 && learnedPreferredTime(t.title, allData.schedulingFeedback) !== null ? " — based on times you've approved for similar tasks" : ""}:
                  </p>
                  {currentCandidate.map(o => <div key={o.id} className="occ-row"><span>{fmtDateShort(o.date)}</span><span>{minutesToTime(o.start)} – {minutesToTime(o.end)}</span></div>)}
                  <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <button className="btn btn-primary btn-sm" onClick={approveCandidate}><Icon name="check" size={13} /> Approve</button>
                    <button className="btn btn-sm" onClick={denyCandidate}>Deny — try another time</button>
                  </div>
                </>
              )}
            </div>
          ...
          ) : !schedPhase && <p style={{ fontSize: 12.5, color: "var(--text-tertiary)" }}>No scheduled blocks yet — use "Suggest schedule" or add one manually.</p>}
...
function TemplateSaveModal({ onSave, onClose }) {
  ...
  return (
    <Modal title="Save today as a template" onClose={onClose}>
      ...
      <p style={{ fontSize: 12.5, color: "var(--text-tertiary)", marginBottom: 14 }}>This saves today's manual events as a reusable layout.</p>
...
function DayTimeline({ blocks, dateStr, settings, onEdit, onStatusChange, onDelete }) {
  const startMinute = Math.floor(timeToMinutes(settings.wakeTime) / 60) * 60;
  const endMinute = Math.ceil(timeToMinutes(settings.bedTime) / 60) * 60;
  const pxPerMin = 1.05;
  const hourCount = Math.max(1, (endMinute - startMinute) / 60);
  const hours = Array.from({ length: hourCount + 1 }, (_, i) => startMinute + i * 60);
  ...
  const posFor = (mins) => Math.max(0, (mins - startMinute) * pxPerMin);
  return (
    <div className="timeline-wrap">
      <div className="timeline-hours">{hours.map(h => <div key={h} className="timeline-hour-label">{minutesToTime(h)}</div>)}</div>
      <div className="timeline-grid" style={{ height: hourCount * 63 }}>
        {hours.map(h => <div key={h} className="timeline-hour-row" />)}
        {isWorkDay && <div className="timeline-band band-work" style={{ top: posFor(workStart), height: Math.max(0, workEnd - workStart) * pxPerMin }}><span>WORK</span></div>}
        {personalWins.map(w => { const ws = timeToMinutes(w.start), we = timeToMinutes(w.end); return <div key={w.id} className="timeline-band band-personal" style={{ top: posFor(ws), height: Math.max(0, we - ws) * pxPerMin }}><span>PERSONAL</span></div>; })}
        {lowEnergyWins.map(w => { const ws = timeToMinutes(w.start), we = timeToMinutes(w.end); return <div key={w.id} className="timeline-band band-lowenergy" style={{ top: posFor(ws), height: Math.max(0, we - ws) * pxPerMin }}><span>LOW ENERGY</span></div>; })}
        {blocks.map((b, i) => { const top = posFor(b.start); const height = Math.max((b.end - b.start) * pxPerMin, 24); const c = b.type === "task" ? urgencyColor(b, dateStr) : categoryColor(b.category); return <div key={b.id + i} onClick={() => onEdit(b)} className={`timeline-block block-${c}`} style={{ top, height }}><div className="timeline-block-title">{b.title}</div>{height > 30 && <div className="timeline-block-time">{minutesToTime(b.start)}–{minutesToTime(b.end)}</div>}</div>; })}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 6 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{streak > 0 && <Tag color="amber"><Icon name="flame" size={11} /> {streak} day{streak > 1 ? "s" : ""}</Tag>}{milestone && <Tag color="green"><Icon name="trophy" size={11} /> {milestone}-day milestone</Tag>}<Tag color="teal">{completedCount}/{applicableDates.length} this month</Tag></div>
      </div>
      <div className="month-grid month-grid-header">{DAY_NAMES.map(d => <div key={d} className="month-day-name">{d}</div>)}</div>
      <div className="month-grid">
        {cells.map((d, i) => {
...
function TaskCard({ t, goals, onUpdate, onDelete, onEditFull, onOpenScheduler }) {
  ...
  return (
    <div className="task-card">
      <div className="task-card-top">
        <EditableCell value={t.title} type="text" onChange={v => onUpdate({ ...t, title: v })} />
        <div className="row-actions"><button className="btn-ghost" onClick={() => onEditFull(t)}><Icon name="edit" /></button><button className="btn-ghost" onClick={() => onDelete(t.id)}><Icon name="trash" /></button></div>
      </div>
      <div className="task-card-row"><span className="task-card-label">Status</span><span className={`status-pill status-${statusColor(t.status)}`}><EditableCell value={t.status} type="select" options={statusOptions} onChange={v => onUpdate({ ...t, status: v }, true)} /></span></div>
      <div className="task-card-row"><span className="task-card-label">Priority</span><span className={`status-pill status-${priorityColor(t.priority)}`}><EditableCell value={t.priority} type="select" options={priorityOptions} onChange={v => onUpdate({ ...t, priority: v })} /></span></div>
      {t.taskType !== "recurring" && <div className="task-card-row"><span className="task-card-label">Due</span><EditableCell value={t.dueDate} type="date" onChange={v => onUpdate({ ...t, dueDate: v })} valueColorVar={`var(--${c}-text)`} /></div>}
      <div className="task-card-row"><span className="task-card-label">Scheduled</span><TaskOccurrenceStrip task={t} onUpdate={onUpdate} onOpenScheduler={onOpenScheduler} /></div>
      <div className="task-card-row"><span className="task-card-label">Est. min</span><EditableCell value={t.estimatedMinutes} type="number" onChange={v => onUpdate({ ...t, estimatedMinutes: v })} /></div>
      {goals.length > 0 && <div className="task-card-row"><span className="task-card-label">Goal</span><select value={t.goalId || ""} onChange={e => onUpdate({ ...t, goalId: e.target.value })} className="cell-select"><option value="">—</option>{goals.map(g => <option key={g.id} value={g.id}>{g.title}</option>)}</select></div>}
    </div>
  );
}
    ...
    <>
      <div className="monday-table-wrap">
        <table className="monday-table">
          <thead><tr><th className="col-task">Task</th><th className="col-status">Status</th><th className="col-priority">Priority</th><th className="col-date">Due date</th><th className="col-scheduled">Scheduled</th><th className="col-min">Est. min</th><th className="col-goal">Goal</th><th className="col-actions"></th></tr></thead>
          <tbody>
            {tasks.map(t => {
              const c = t.dueDate ? urgencyColor(t, todayStr()) : "slate";
              return (
                <tr key={t.id}>
                  <td className="col-task"><div className="cell-wrap"><EditableCell value={t.title} type="text" onChange={v => onUpdate({ ...t, title: v })} />{t.taskType === "recurring" && <Tag color="indigo">recurring</Tag>}</div></td>
                  <td className="col-status"><span className={`status-pill status-${statusColor(t.status)}`}><EditableCell value={t.status} type="select" options={statusOptions} onChange={v => onUpdate({ ...t, status: v }, true)} /></span></td>
                  <td className="col-priority"><span className={`status-pill status-${priorityColor(t.priority)}`}><EditableCell value={t.priority} type="select" options={priorityOptions} onChange={v => onUpdate({ ...t, priority: v })} /></span></td>
                  <td className="col-date">{t.taskType === "recurring" ? <span style={{ color: "var(--text-tertiary)", fontSize: 12.5 }}>—</span> : <EditableCell value={t.dueDate} type="date" onChange={v => onUpdate({ ...t, dueDate: v })} valueColorVar={`var(--${c}-text)`} />}</td>
                  <td className="col-scheduled"><TaskOccurrenceStrip task={t} onUpdate={onUpdate} onOpenScheduler={onOpenScheduler} /></td>
                  <td className="col-min"><EditableCell value={t.estimatedMinutes} type="number" onChange={v => onUpdate({ ...t, estimatedMinutes: v })} /></td>
                  <td className="col-goal"><select value={t.goalId || ""} onChange={e => onUpdate({ ...t, goalId: e.target.value })} className="cell-select"><option value="">—</option>{goals.map(g => <option key={g.id} value={g.id}>{g.title}</option>)}</select></td>
                  <td className="col-actions"><div className="cell-actions"><button className="btn-ghost" onClick={() => onEditFull(t)}><Icon name="edit" /></button><button className="btn-ghost" onClick={() => onDelete(t.id)}><Icon name="trash" /></button></div></td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="task-card-list">{tasks.map(t => <TaskCard key={t.id} t={t} goals={goals} onUpdate={onUpdate} onDelete={onDelete} onEditFull={onEditFull} onOpenScheduler={onOpenScheduler} />)}</div>
    </>
  );
}
...
function CompletedTaskRow({ t, onRestore, onDeletePermanently }) {
  ...
  return (
    <div className="block-row">
      <div className={`block-stripe stripe-${c}`} />
      <div style={{ flex: 1, minWidth: 0 }}><p className="block-title" style={{ textDecoration: "line-through", opacity: 0.75 }}>{t.title}</p><div className="block-tags"><Tag color={c}>{t.category}</Tag>{t.completedAt && <Tag>completed {fmtDateShort(t.completedAt.slice(0, 10))}</Tag>}</div></div>
      <div className="row-actions"><button className="btn btn-sm" onClick={() => onRestore(t.id)}>Move back to active</button><button className="btn-ghost" onClick={() => onDeletePermanently(t.id)}><Icon name="trash" /></button></div>
    </div>
  );
}
...
function InboxTab({ inbox, onAdd, onTriage, onDelete }) {
  ...
  return (
    <div>
      <div className="hero-banner"><h2>Quick capture</h2><p>Dump anything here the moment it crosses your mind — sort it into a real task later, whenever you have a minute.</p></div>
      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => { if (e.key === "Enter") add(); }} placeholder="Type a thought, idea, or to-do and hit Enter…" autoFocus /><button className="btn btn-primary" onClick={add}><Icon name="plus" size={14} /> Add</button></div>
      {inbox.length === 0 ? <div className="empty-state"><p>Nothing captured yet — your inbox is clear.</p></div> : inbox.slice().reverse().map(item => (
        <div key={item.id} className="block-row">
          ...
          <div className="row-actions"><button className="btn btn-sm" onClick={() => onTriage(item, "personal")}>→ Personal</button><button className="btn btn-sm" onClick={() => onTriage(item, "work")}>→ Work</button><button className="btn-ghost" onClick={() => onDelete(item.id)}><Icon name="trash" /></button></div>
        </div>
      ))}
    </div>
  );
...
function MainApp({ session }) {
  ...
  const updateTask = (task, isStatusChange) => { if (isStatusChange && task.status === "done") { setCompleteConfirm(task); return; } update({ tasks: data.tasks.map(t => t.id === task.id ? task : t) }); };
  ...
  const recordSchedulingFeedback = (fb) => update({ schedulingFeedback: [...data.schedulingFeedback, fb].slice(-300) });
  ...
  const addGoalFromModal = (goal, addStarterTasks) => { const newTasks = addStarterTasks ? (GOAL_TEMPLATE_TASKS[goal.category] || GOAL_TEMPLATE_TASKS.personal).map(title => ({ id: uid(), title, category: goal.category, taskType: "one_time", dueDate: addDays(todayStr(), 7), targetDate: "", estimatedMinutes: 20, priority: "medium", status: "not_started", notes: "", goalId: goal.id, occurrences: [], avoidLowEnergy: true })) : []; update({ goals: [...data.goals, goal], tasks: [...data.tasks, ...newTasks] }); setGoalModal(null); };
  ...
  const handleBlockDelete = (block) => { if (block.taskId) { const t = data.tasks.find(t => t.id === block.taskId); if (t && block.occurrenceId) updateTask({ ...t, occurrences: (t.occurrences || []).filter(o => o.id !== block.occurrenceId) }); } else if (block.type === "event") deleteEvent(block.eventId || block.id); };
  const handleBlockEdit = (block) => { if (block.type === "event") { const ev = data.events.find(e => e.id === block.eventId); if (ev) setEventModal(ev); } else if (block.taskId) { const t = data.tasks.find(t => t.id === block.taskId); if (t) { setJumpToScheduler(false); setTaskModal(t); } } else if (block.habitId) { const hh = data.habits.find(hh => hh.id === block.habitId); if (hh) setHabitModal(hh); } };
  ...
  const triageInboxItem = (item, category) => { const newTask = { id: uid(), title: item.text, category, taskType: "one_time", dueDate: addDays(todayStr(), 3), targetDate: "", estimatedMinutes: 30, priority: "medium", status: "not_started", notes: "", goalId: "", occurrences: [], avoidLowEnergy: true }; update({ tasks: [...data.tasks, newTask], inbox: data.inbox.filter(i => i.id !== item.id) }); };

  const saveAsTemplate = (name) => { const todaysEvents = data.events.filter(e => e.date === viewDate); update({ scheduleTemplates: [...data.scheduleTemplates, { id: uid(), name, events: todaysEvents.map(e => ({ title: e.title, start: e.start, end: e.end, category: e.category })) }] }); setShowTemplateModal(false); };
  const applyTemplate = (template, targetDate) => { const newEvents = template.events.map(e => ({ id: uid(), title: e.title, date: targetDate, start: e.start, end: e.end, category: e.category, recurring: false })); update({ events: [...data.events, ...newEvents] }); };
      ...
      <div className="main">
        <div className="page-header">
          ...
          <div>
            ...
            <p className="page-subtitle">{activeTab === "today" ? "Your active schedule, habits, and notes for the day." : activeTab === "calendar" ? "Month, week, or day — with work, personal, low-energy, and sleep hours shown." : activeTab === "tasks" ? "Click any cell to edit it directly." : activeTab === "completed" ? "Everything you've finished, color-coded by category." : activeTab === "goals" ? "Update progress daily — every change is time-stamped." : activeTab === "habits" ? "Exact times, calendar history, streaks, and milestones." : activeTab === "fitness" ? "Adaptive training, progress tracking, and nutrition." : activeTab === "summary" ? "How consistent you've actually been, across everything." : activeTab === "inbox" ? "Capture first, organize later." : "Data-driven observations on how things are going."}</p>
          </div>
          <div className="header-actions">
            {activeTab === "today" && <button className="btn btn-sm" onClick={() => setShowTemplateModal(true)}><Icon name="template" size={14} /><span className="hide-xs"> Save as template</span></button>}
            {activeTab === "today" && <button className="btn btn-sm" onClick={printAgenda}><Icon name="print" size={14} /><span className="hide-xs"> Print agenda</span></button>}
            <button className="icon-toggle" onClick={toggleTheme} title="Toggle theme"><Icon name={data.theme === "dark" ? "sun" : "moon"} /></button>
          </div>
        </div>

        {pendingSuggestionTasks.length > 0 && activeTab !== "tasks" && (<div className="insight-row insight-warn" style={{ marginBottom: 18 }}><Icon name="warn" /><p>{pendingSuggestionTasks.length} existing task{pendingSuggestionTasks.length > 1 ? "s" : ""} need a schedule approved. <button className="btn-ghost" style={{ textDecoration: "underline", padding: 0, fontWeight: 700, color: "var(--amber-text)" }} onClick={() => setActiveTab("tasks")}>Go to Tasks</button></p></div>)}

        {activeTab === "today" && (
          <div>
            {latestReview && (<div className="hero-banner" style={{ marginBottom: 18 }}><h2>Weekly review — week of {fmtDateShort(latestReview.weekStart)}</h2><p style={{ marginBottom: 8 }}>{latestReview.tasksCompleted} task{latestReview.tasksCompleted !== 1 ? "s" : ""} completed · {latestReview.workouts} workout{latestReview.workouts !== 1 ? "s" : ""} logged{latestReview.strongHabits.length > 0 ? ` · strong on: ${latestReview.strongHabits.slice(0, 2).join(", ")}` : ""}</p>{latestReview.focusAreas.map((f, i) => <p key={i} style={{ margin: "4px 0", fontSize: 13 }}>• {f}</p>)}</div>)}
            ...
            {blocks.length === 0 ? <div className="empty-state"><p>Nothing scheduled yet — open the Calendar tab to see the full day grid, or add a task/habit/event to get started.</p></div> : blocks.map((b, i) => <ScheduleBlock key={b.id + i} block={b} dateStr={viewDate} onEdit={handleBlockEdit} onStatusChange={markTaskDone} onDelete={handleBlockDelete} />)}
        ...
        {activeTab === "calendar" && (
          <div>
            ...
            {data.scheduleTemplates.length > 0 && calView === "day" && (<div style={{ marginBottom: 14, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}><span style={{ fontSize: 12, color: "var(--text-tertiary)", fontWeight: 700 }}>Apply template:</span>{data.scheduleTemplates.map(t => <button key={t.id} className="btn btn-sm" onClick={() => applyTemplate(t, viewDate)}><Icon name="template" size={12} /> {t.name}</button>)}</div>)}
        ...
        {activeTab === "completed" && (
          <div>
            {completedTasks.length === 0 ? <div className="empty-state"><p>Nothing completed yet — finished tasks will show up here, color-coded by category.</p></div> :
              ["personal", "work", "fitness"].map(cat => { const items = completedTasks.filter(t => t.category === cat); if (items.length === 0) return null; return (<div key={cat} style={{ marginBottom: 22 }}><h3 className="section-heading" style={{ textTransform: "capitalize" }}>{cat}</h3>{items.map(t => <CompletedTaskRow key={t.id} t={t} onRestore={restoreTask} onDeletePermanently={deleteTaskPermanently} />)}</div>); })}
          </div>
        )}
        ...
        {taskModal !== null && <TaskModal task={taskModal.id ? taskModal : null} defaultCategory={taskFilter} goals={data.goals} allData={data} jumpToScheduler={jumpToScheduler} onRecordFeedback={recordSchedulingFeedback} onSave={saveTaskFromModal} onClose={() => { setTaskModal(null); setJumpToScheduler(false); }} />}
