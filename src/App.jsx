...
function emptyData() {
  return { settings: DEFAULT_SETTINGS, tasks: [], goals: [], habits: [], events: [], dailyLogs: {}, completedLog: [], workoutLog: [], bodyStats: [], exerciseHistory: [], currentPlan: null, theme: "dark" };
}
...
// Suggest candidate occurrences (date+start+end) for a task, given everything currently fixed (events, habits, other approved occurrences).
function suggestOccurrencesForTask(task, allData) {
  ...
  const perDayMinutes = Math.max(15, Math.round(totalMinutes / chosenDays.length));
  ...
  chosenDays.forEach(d => {
    ...
    if (task.category === "work") {
      if (!(settings.workDays || [1, 2, 3, 4, 5]).includes(dow)) return;
    ...
    const start = findOpenSlot(perDayMinutes, rangeStart, rangeEnd, occupied) ?? findOpenSlot(perDayMinutes, timeToMinutes(settings.wakeTime), timeToMinutes(settings.bedTime), occupied);
    if (start !== null) occurrences.push({ id: uid(), date: d, start, end: start + perDayMinutes });
  });
  return occurrences;
}

function findOpenSlot(duration, rangeStart, rangeEnd, occupied) {
  const sorted = [...occupied].sort((a, b) => a.start - b.start);
  let cursor = rangeStart;
  for (const slot of sorted) { if (slot.start - cursor >= duration && cursor >= rangeStart && !overlapsAny(cursor, cursor + duration, occupied)) return cursor; cursor = Math.max(cursor, slot.end); }
  if (rangeEnd - cursor >= duration && !overlapsAny(cursor, cursor + duration, occupied)) return cursor;
  return null;
}

// Fixed, non-negotiable items for a given day: events + habits (used as "occupied" baseline before placing task occurrences)
function getFixedOccupiedForDay(dateStr, allData) {
  ...
  habits.filter(h => h.active && (h.frequency === "daily" || (h.frequency === "weekly" && h.days.includes(dow)))).forEach(h => {
    if (h.time) { const s = timeToMinutes(h.time); occ.push({ start: s, end: s + (h.duration || 20) }); }
  });
  return occ;
}

// Build the full schedule for one day: events + habits + approved task occurrences (recurring + one-time), with conflict-driven rescheduling.
function autoScheduleDay(dateStr, allData) {
  ...
  const scheduled = [];
  const occupied = [];
  events.filter(e => eventOccursOn(e, dateStr)).forEach(e => {
    const block = { id: e.id + "-" + dateStr, eventId: e.id, type: "event", title: e.title, start: timeToMinutes(e.start), end: timeToMinutes(e.end), category: e.category || "work", locked: true };
    occupied.push({ start: block.start, end: block.end }); scheduled.push(block);
  });
  habits.filter(h => h.active && (h.frequency === "daily" || (h.frequency === "weekly" && h.days.includes(dow)))).forEach(h => {
    const duration = h.duration || 20;
    let start = h.time ? timeToMinutes(h.time) : null;
  ...
  tasks.filter(t => t.taskType === "recurring" && recurringTaskAppliesOn(t, dateStr)).forEach(t => {
    ...
    let rangeStart = dayStart, rangeEnd = dayEnd;
    ...
    let start = findOpenSlot(duration, rangeStart, rangeEnd, occupied);
    if (start === null) start = findOpenSlot(duration, dayStart, dayEnd, occupied);
  ...
  // One-time tasks: use their APPROVED occurrences (stored, editable), auto-bump if they collide with a fixed item (event/habit) added after approval.
  tasks.filter(t => t.taskType !== "recurring" && t.status !== "done" && t.occurrences).forEach(t => {
    (t.occurrences || []).filter(o => o.date === dateStr).forEach(o => {
      ...
      if (overlapsAny(start, end, occupied)) {
        const bumped = findOpenSlot(duration, dayStart, dayEnd, occupied);
        if (bumped !== null) { start = bumped; end = bumped + duration; }
      }
...
function PersonalWindowRow({ w, onChange, onDelete }) {
  ...
  return (
    <div className="pw-row">
      <input className="pw-label-input" value={w.label} onChange={e => onChange({ ...w, label: e.target.value })} placeholder="Label (e.g. Weekday evenings)" />
...
function OnboardingModal({ settings, onSave, onClose }) {
  const [s, setS] = useState({ ...DEFAULT_SETTINGS, ...settings, personalWindows: settings.personalWindows && settings.personalWindows.length ? settings.personalWindows : DEFAULT_PERSONAL_WINDOWS });
    ...
    <Modal title="Schedule settings" onClose={onClose} wide>
      ...
      <Field label="Sleep">
        <div className="field-row">
          <div><label className="sublabel">Wake time</label><input type="time" value={s.wakeTime} onChange={e => setS({ ...s, wakeTime: e.target.value })} /></div>
          <div><label className="sublabel">Bed time</label><input type="time" value={s.bedTime} onChange={e => setS({ ...s, bedTime: e.target.value })} /></div>
        </div>
      </Field>
      <Field label="Work hours">
        <div className="field-row">
          <div><label className="sublabel">Start</label><input type="time" value={s.workStart} onChange={e => setS({ ...s, workStart: e.target.value })} /></div>
          <div><label className="sublabel">End</label><input type="time" value={s.workEnd} onChange={e => setS({ ...s, workEnd: e.target.value })} /></div>
        </div>
      </Field>
      ...
      <Field label="Personal time windows">
        <p style={{ fontSize: 12, color: "var(--text-tertiary)", margin: "0 0 8px" }}>Add as many separate blocks as you need — mornings, evenings, full weekends, etc.</p>
...
function TaskModal({ task, onSave, onClose, defaultCategory, goals, allData }) {
  const [t, setT] = useState(task || { id: uid(), title: "", category: defaultCategory || "personal", taskType: "one_time", dueDate: todayStr(), targetDate: "", estimatedMinutes: 30, priority: "medium", status: "not_started", notes: "", goalId: "", recurFrequency: "daily", recurDays: [1, 2, 3, 4, 5], occurrences: [] });
  ...
  return (
    <Modal title={task ? "Edit task" : "New task"} onClose={onClose} wide>
      ...
      {t.taskType === "one_time" ? (
        ...
        <div>
          <Field label="Repeats"><select value={t.recurFrequency} onChange={e => setT({ ...t, recurFrequency: e.target.value })}><option value="daily">Every day</option><option value="weekly">Specific days</option></select></Field>
      ...
      {t.taskType === "one_time" && (
        <div className="occ-section">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: 0.3 }}>Scheduled days & times</label>
            <button className="btn btn-sm" onClick={runSuggestion}><Icon name="wand" size={13} /> Suggest schedule</button>
          </div>

          {suggested && (
            <div className="suggestion-box">
              <p style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 6 }}>Suggested time blocks:</p>
              {suggested.length === 0 ? <p style={{ fontSize: 12.5, color: "var(--text-tertiary)" }}>No open slots found — try a longer target date or shorter estimate.</p> :
                suggested.map(o => <div key={o.id} className="occ-row"><span>{fmtDateShort(o.date)}</span><span>{minutesToTime(o.start)} – {minutesToTime(o.end)}</span></div>)}
              {suggested.length > 0 && (
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button className="btn btn-primary btn-sm" onClick={approveSuggestion}><Icon name="check" size={13} /> Approve</button>
                  <button className="btn btn-sm" onClick={denySuggestion}>Deny</button>
                </div>
              )}
            </div>
          )}
    ...
    <div className="timeline-wrap">
      ...
      <div className="timeline-grid">
        ...
        {blocks.map((b, i) => {
          ...
          return <div key={b.id + i} onClick={() => onEdit(b)} className={`timeline-block block-${c}`} style={{ top, height }}><div className="timeline-block-title">{b.title}</div>{height > 30 && <div className="timeline-block-time">{minutesToTime(b.start)}–{minutesToTime(b.end)}</div>}</div>;
        })}
      </div>
    </div>
  );
...
function HabitCalendar({ habit, completedLog, onToggle, monthAnchor }) {
  ...
  return (
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><p style={{ fontWeight: 700, fontSize: 15, margin: 0 }}>{habit.name}</p><Tag color="teal">{completedCount}/{applicableDates.length} this month</Tag></div>
      <div className="month-grid month-grid-header">{DAY_NAMES.map(d => <div key={d} className="month-day-name">{d}</div>)}</div>
      <div className="month-grid">
        {cells.map((d, i) => {
          if (!d) return <div key={i} />;
...
function TaskOccurrenceStrip({ task, onUpdate }) {
  ...
  if (occs.length === 0) return <span style={{ fontSize: 12, color: "var(--coral-text)" }}>not scheduled</span>;
    ...
    <div className="occ-strip">
      {occs.map(o => (
        <div key={o.id} className="occ-chip">
          <input type="date" value={o.date} onChange={e => updateOcc(o.id, { date: e.target.value })} className="occ-chip-input" />
...
function TaskTable({ tasks, goals, onUpdate, onDelete, onEditFull }) {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table className="monday-table">
        <thead><tr><th style={{ minWidth: 180 }}>Task</th><th>Status</th><th>Priority</th><th>Due date</th><th style={{ minWidth: 160 }}>Scheduled</th><th>Est. min</th><th>Goal</th><th></th></tr></thead>
        <tbody>
          {tasks.map(t => {
            ...
            return (
              <tr key={t.id}>
                ...
                <td><TaskOccurrenceStrip task={t} onUpdate={onUpdate} /></td>
    ...
    <div className="card">
      <div className="goal-card-progress">
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}><span className="goal-target">Target:</span><input type="date" value={g.targetDate} onChange={e => onUpdate({ ...g, targetDate: e.target.value })} className="cell-input" style={{ width: "auto" }} /></div>
        </div>
  ...
  return (
    <div className="card">
      ...
      <table className="workout-table">
        ...
        <tbody>{workout.main.map(ex => (<tr key={ex.id} className={ex.completed ? "row-done" : ""}><td><EditableCell value={ex.name} type="text" onChange={v => onUpdateExercise(ex.id, { ...ex, name: v })} /></td><td><EditableCell value={ex.sets} type="number" onChange={v => onUpdateExercise(ex.id, { ...ex, sets: v })} /></td><td><EditableCell value={ex.reps} type="text" onChange={v => onUpdateExercise(ex.id, { ...ex, reps: v })} /></td><td><EditableCell value={ex.weight} type="text" onChange={v => onUpdateExercise(ex.id, { ...ex, weight: v })} /></td><td><input type="checkbox" checked={ex.completed} onChange={() => onUpdateExercise(ex.id, { ...ex, completed: !ex.completed })} className="checkbox-lg" /></td></tr>))}</tbody>
      </table>
...
function SummaryTab({ data }) {
  ...
  return (
    <div>
      ...
      {habitStats.length === 0 ? <p style={{ fontSize: 13.5, color: "var(--text-secondary)" }}>No active habits yet.</p> : habitStats.map((h, i) => (<div key={i} className="block-row"><div className={"block-stripe " + (h.rate >= 70 ? "stripe-green" : h.rate >= 40 ? "stripe-amber" : "stripe-coral")} /><div style={{ flex: 1 }}><p className="block-title">{h.name}</p><div className="progress-track" style={{ marginTop: 4 }}><div className="progress-fill" style={{ width: `${h.rate}%` }} /></div></div><span style={{ fontSize: 13, fontWeight: 800, minWidth: 70, textAlign: "right" }}>{h.completed}/{h.applicable}</span></div>))}
    </div>
  );
}
...
function MainApp({ session }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // One-time migration: give existing tasks without occurrences a suggested schedule to approve.
      if (!loadedData.settings.scheduleMigrationDone) {
        loadedData = {
          ...loadedData,
          tasks: loadedData.tasks.map(t => {
            if (t.taskType === "recurring" || t.status === "done" || (t.occurrences && t.occurrences.length)) return t;
            return { ...t, pendingSuggestion: true };
          }),
          settings: { ...loadedData.settings, scheduleMigrationDone: true },
        };
  ...
  const pendingSuggestionTasks = useMemo(() => data.tasks.filter(t => t.pendingSuggestion), [data.tasks]);
  ...
  const saveTaskFromModal = (task) => { const exists = data.tasks.some(t => t.id === task.id); const cleaned = { ...task, pendingSuggestion: false }; update({ tasks: exists ? data.tasks.map(t => t.id === task.id ? cleaned : t) : [...data.tasks, cleaned] }); setTaskModal(null); };
  const addTaskQuick = () => setTaskModal({ id: uid(), title: "", category: taskFilter, taskType: "one_time", dueDate: todayStr(), targetDate: "", estimatedMinutes: 30, priority: "medium", status: "not_started", notes: "", goalId: "", occurrences: [] });
      ...
      <div className="main">
        <div className="page-header">
          ...
          <div>
            ...
            <p className="page-subtitle">{activeTab === "today" ? "Your day, mapped out by time and priority." : activeTab === "calendar" ? "Month, week, or day — with work, personal, and sleep hours shown." : activeTab === "tasks" ? "Click any cell to edit it directly." : activeTab === "goals" ? "Update progress daily — every change is time-stamped." : activeTab === "habits" ? "Exact times, calendar history, and completion tracking." : activeTab === "fitness" ? "Adaptive training, progress tracking, and nutrition." : activeTab === "summary" ? "How consistent you've actually been, across everything." : "Data-driven observations on how things are going."}</p>
          </div>
        ...
        {pendingSuggestionTasks.length > 0 && activeTab !== "tasks" && (
          <div className="insight-row insight-warn" style={{ marginBottom: 18 }}>
            <Icon name="warn" /><p>{pendingSuggestionTasks.length} existing task{pendingSuggestionTasks.length > 1 ? "s" : ""} need a schedule approved under the new system. <button className="btn-ghost" style={{ textDecoration: "underline", padding: 0, fontWeight: 700, color: "var(--amber-text)" }} onClick={() => setActiveTab("tasks")}>Go to Tasks</button></p>
          </div>
        )}
        ...
        {activeTab === "calendar" && (
          <div>
            ...
            <div className="legend-row"><span className="legend-chip"><span className="legend-dot dot-indigo-bg" /> Work hours</span><span className="legend-chip"><span className="legend-dot dot-coral-bg" /> Personal hours</span><span className="legend-chip"><span className="legend-dot dot-slate-bg" /> Sleep</span></div>
        ...
        {activeTab === "tasks" && (
          <div>
            ...
            {tasksForFilter.length === 0 ? <div className="empty-state"><p>No {taskFilter} tasks yet.</p></div> : <TaskTable tasks={tasksForFilter} goals={data.goals} onUpdate={updateTask} onDelete={deleteTask} onEditFull={setTaskModal} />}
          </div>
        )}
        ...
        {activeTab === "habits" && (
          <div>
            ...
            {data.habits.length === 0 ? <div className="empty-state"><p>No habits yet.</p></div> : (
              <div>
                {data.habits.map(hh => (<div key={hh.id} className="card" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px" }}><div style={{ flex: 1 }}><p style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>{hh.name}</p><div style={{ display: "flex", gap: 6, marginTop: 6 }}><Tag>{hh.frequency === "daily" ? "Every day" : "Weekly"}</Tag><Tag>{hh.duration}m</Tag><Tag color="indigo">{hh.time}</Tag>{!hh.active && <Tag color="coral">paused</Tag>}</div></div><button className="btn btn-sm" onClick={() => saveHabit({ ...hh, active: !hh.active })}>{hh.active ? "Pause" : "Resume"}</button><div className="row-actions"><button className="btn-ghost" onClick={() => setHabitModal(hh)}><Icon name="edit" /></button><button className="btn-ghost" onClick={() => deleteHabit(hh.id)}><Icon name="trash" /></button></div></div>))}
        ...
        {taskModal !== null && <TaskModal task={taskModal.id ? taskModal : null} defaultCategory={taskFilter} goals={data.goals} allData={data} onSave={saveTaskFromModal} onClose={() => setTaskModal(null)} />}
      </div>
