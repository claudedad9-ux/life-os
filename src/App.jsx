...
const DEFAULT_SETTINGS = {
  ...
  personalStart: "17:30", personalEnd: "21:30",
...
// Which calendar days a ONE-TIME task's remaining estimated time should be spread across.
function spreadTaskDays(task, fromDate) {
  const endDate = task.targetDate || task.dueDate;
  if (!endDate || endDate < fromDate) return [fromDate];
  const allDays = dateRangeDays(fromDate, endDate);
  if (allDays.length <= 1) return [fromDate];
  ...
  return chosen.length ? chosen : [fromDate];
}
...
function eventOccursOn(event, dateStr) {
  ...
  const dow = new Date(dateStr + "T00:00:00").getDay();
  const startDow = new Date(event.date + "T00:00:00").getDay();
...
function autoScheduleDay(dateStr, state) {
  ...
  const personalStart = timeToMinutes(settings.personalStart || "17:30"), personalEnd = timeToMinutes(settings.personalEnd || "21:30");
  const dow = new Date(dateStr + "T00:00:00").getDay();
  const isWorkDay = (settings.workDays || [1, 2, 3, 4, 5]).includes(dow);
  const today = todayStr();

  const fixed = [];
  events.filter(e => eventOccursOn(e, dateStr)).forEach(e => fixed.push({ id: e.id + "-" + dateStr, eventId: e.id, type: "event", title: e.title, start: timeToMinutes(e.start), end: timeToMinutes(e.end), category: e.category || "work", locked: true }));
  ...
  fixed.filter(f => f.type === "habit").forEach(hb => {
    let start;
    if (hb.preferredStart !== null && hb.preferredStart !== undefined) start = !overlaps(hb.preferredStart, hb.preferredStart + hb.duration) ? hb.preferredStart : findSlot(hb.duration, dayStart, dayEnd);
  ...
  function rangeForCategory(category, partOfDay) {
    if (category === "work") return [workStart, workEnd];
    if (category === "personal" && partOfDay === "evening") return [Math.max(personalStart, dayStart), Math.min(personalEnd, dayEnd)];
    if (partOfDay && PART_OF_DAY_RANGES[partOfDay]) { const [rs, re] = PART_OF_DAY_RANGES[partOfDay]; return [Math.max(rs, dayStart), Math.min(re, dayEnd)]; }
    return [dayStart, dayEnd];
  }

  // Recurring tasks (daily checklist-style items)
  tasks.filter(t => t.taskType === "recurring" && recurringTaskAppliesOn(t, dateStr)).forEach(t => {
    const duration = t.estimatedMinutes || 15;
    const [rangeStart, rangeEnd] = rangeForCategory(t.category, t.partOfDay);
    let start = findSlot(duration, rangeStart, rangeEnd);
    if (start === null) start = findSlot(duration, dayStart, dayEnd);
    if (start !== null) {
      const block = { id: "rtask-" + t.id + "-" + dateStr, taskId: t.id, type: "task", title: t.title, start, end: start + duration, category: t.category, priority: t.priority, dueDate: null, status: t.status, manual: false, recurring: true };
      occupied.push({ start: block.start, end: block.end }); scheduled.push(block);
    }
  });

  // One-time tasks
  const oneTimeTasks = tasks.filter(t => t.taskType !== "recurring" && t.status !== "done").sort((a, b) => { const pr = { high: 0, medium: 1, low: 2 }; return pr[a.priority] - pr[b.priority]; });
  oneTimeTasks.forEach(t => {
    const fromDate = today < dateStr ? dateStr : today;
    const days = spreadTaskDays(t, fromDate);
    if (!days.includes(dateStr)) return;
    const totalMinutes = t.estimatedMinutes || 30;
    const duration = Math.max(15, Math.round(totalMinutes / Math.max(days.length, 1)));
    if (t.category === "work" && !isWorkDay) return;
    const [rangeStart, rangeEnd] = rangeForCategory(t.category, t.partOfDay);
    let start = findSlot(duration, rangeStart, rangeEnd);
    if (start === null && t.category !== "work") start = findSlot(duration, dayStart, dayEnd);
    if (start !== null) {
      const block = { id: "task-" + t.id + "-" + dateStr, taskId: t.id, type: "task", title: t.title, start, end: start + duration, category: t.category, priority: t.priority, dueDate: t.dueDate, status: t.status, manual: false };
      occupied.push({ start: block.start, end: block.end }); scheduled.push(block);
    }
  });
...
// Where (which date + time) a one-time task is CURRENTLY scheduled, for display in the Tasks table.
function getTaskSchedule(task, allData) {
  if (task.taskType === "recurring" || task.status === "done") return null;
  const today = todayStr();
  const fromDate = today < task.dueDate ? today : today;
  const days = spreadTaskDays(task, fromDate);
  for (const d of days) {
    const dayBlocks = autoScheduleDay(d, allData);
    const match = dayBlocks.find(b => b.taskId === task.id);
    if (match) return { date: d, start: match.start, end: match.end };
  }
  return null;
}
...
function Icon({ name, size = 16 }) {
  ...
  switch (name) {
    ...
    case "repeat": return <svg style={s} viewBox="0 0 24 24" fill="none"><path d="M17 2l4 4-4 4M7 22l-4-4 4-4M3 6h12a4 4 0 014 4M21 18H9a4 4 0 01-4-4" stroke={stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>;
    default: return null;
  }
}
    ...
    <Modal title="Schedule settings" onClose={onClose} wide>
      ...
      <Field label="Sleep">
        <div className="field-row">
          <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>Wake time</label><input type="time" value={s.wakeTime} onChange={e => setS({ ...s, wakeTime: e.target.value })} /></div>
          <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>Bed time</label><input type="time" value={s.bedTime} onChange={e => setS({ ...s, bedTime: e.target.value })} /></div>
        </div>
      </Field>
      <Field label="Work hours">
        <div className="field-row">
          <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>Start</label><input type="time" value={s.workStart} onChange={e => setS({ ...s, workStart: e.target.value })} /></div>
          <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>End</label><input type="time" value={s.workEnd} onChange={e => setS({ ...s, workEnd: e.target.value })} /></div>
        </div>
      </Field>
      ...
      <Field label="Personal time hours">
        <div className="field-row">
          <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>Start</label><input type="time" value={s.personalStart} onChange={e => setS({ ...s, personalStart: e.target.value })} /></div>
          <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>End</label><input type="time" value={s.personalEnd} onChange={e => setS({ ...s, personalEnd: e.target.value })} /></div>
        </div>
      </Field>
  ...
  return (
    <Modal title={event ? "Edit event" : "New meeting / appointment"} onClose={onClose}>
      ...
      <Field label="Recurrence">
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, fontWeight: 600, marginBottom: e.recurring ? 10 : 0 }}>
          <input type="checkbox" checked={e.recurring} onChange={ev => setE({ ...e, recurring: ev.target.checked })} style={{ width: 17, height: 17 }} /> This repeats weekly
        </label>
        {e.recurring && (
          <div className="field-row" style={{ marginTop: 8 }}>
            <div><label style={{ fontSize: 11.5, color: "var(--text-tertiary)" }}>Ends on (optional)</label><input type="date" value={e.recurEnd} onChange={ev => setE({ ...e, recurEnd: ev.target.value })} /></div>
            <div style={{ display: "flex", alignItems: "flex-end" }}><p style={{ fontSize: 12, color: "var(--text-tertiary)", margin: "0 0 9px" }}>Leave blank for an open-ended weekly meeting.</p></div>
          </div>
        )}
      </Field>
...
function TaskModal({ task, onSave, onClose, defaultCategory, goals }) {
  const [t, setT] = useState(task || { id: uid(), title: "", category: defaultCategory || "personal", taskType: "one_time", dueDate: todayStr(), targetDate: "", estimatedMinutes: 30, priority: "medium", partOfDay: "midday", status: "not_started", notes: "", goalId: "", recurFrequency: "daily", recurDays: [1, 2, 3, 4, 5] });
    ...
    <Modal title={task ? "Edit task" : "New task"} onClose={onClose}>
      ...
      <Field label="Type">
        <div className="day-pill-row">
          <button className={"day-pill" + (t.taskType === "one_time" ? " active" : "")} onClick={() => setT({ ...t, taskType: "one_time" })}>One-time (has a due date)</button>
          <button className={"day-pill" + (t.taskType === "recurring" ? " active" : "")} onClick={() => setT({ ...t, taskType: "recurring" })}>Recurring (e.g. email, lunch)</button>
        </div>
      </Field>
      <div className="field-row">
        <Field label="Category"><select value={t.category} onChange={e => setT({ ...t, category: e.target.value })}><option value="work">Work</option><option value="personal">Personal</option><option value="fitness">Fitness</option></select></Field>
        <Field label="Priority"><select value={t.priority} onChange={e => setT({ ...t, priority: e.target.value })}><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></Field>
      </div>
      {t.taskType === "one_time" ? (
        <div className="field-row">
          <Field label="Due date"><input type="date" value={t.dueDate} onChange={e => setT({ ...t, dueDate: e.target.value })} /></Field>
          <Field label="Target completion (optional)"><input type="date" value={t.targetDate} onChange={e => setT({ ...t, targetDate: e.target.value })} /></Field>
        </div>
      ) : (
        <div>
          <Field label="Repeats">
            <select value={t.recurFrequency} onChange={e => setT({ ...t, recurFrequency: e.target.value })}><option value="daily">Every day</option><option value="weekly">Specific days</option></select>
          </Field>
          {t.recurFrequency === "weekly" && <Field label="Days"><div className="day-pill-row">{DAY_NAMES.map((d, i) => <button key={i} className={"day-pill" + ((t.recurDays || []).includes(i) ? " active" : "")} onClick={() => toggleDay(i)}>{d}</button>)}</div></Field>}
        </div>
      )}
      <div className="field-row">
        ...
        <Field label="Preferred part of day"><select value={t.partOfDay} onChange={e => setT({ ...t, partOfDay: e.target.value })}><option value="morning">Morning</option><option value="midday">Midday</option><option value="evening">Evening (personal hours)</option><option value="night">Night</option></select></Field>
      </div>
      <Field label="Status"><select value={t.status} onChange={e => setT({ ...t, status: e.target.value })}><option value="not_started">Not started</option><option value="in_progress">In progress</option><option value="blocked">Blocked</option><option value="done">Done</option></select></Field>
...
function BlockDetailModal({ block, dateStr, onClose, onSaveEvent, onSaveTask, onDeleteEvent }) {
  if (!block) return null;
  if (block.type === "event") {
    return <EventModal event={{ id: block.eventId, title: block.title, date: dateStr, start: minutesToTime(block.start).replace(/ (AM|PM)/, ""), end: minutesToTime(block.end).replace(/ (AM|PM)/, ""), category: block.category }} onSave={onSaveEvent} onClose={onClose} />;
  }
  return null;
}
    ...
    <div className="block-row">
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="block-tags">
          ...
          {block.type === "task" && !block.recurring && <Tag color={c}>{c === "coral" ? "urgent" : c === "amber" ? "soon" : "on track"}</Tag>}
          {block.recurring && <Tag color="indigo">recurring</Tag>}
  ...
  return (
    <div className="timeline-wrap">
      ...
      <div className="timeline-grid">
        ...
        <div className="timeline-band band-personal" style={{ top: (personalStart - startHour * 60) * pxPerMin, height: (personalEnd - personalStart) * pxPerMin }}><span>PERSONAL</span></div>
        {blocks.map((b, i) => {
...
function TaskTable({ tasks, goals, allData, onUpdate, onDelete, onEditFull, dateStr }) {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table className="monday-table">
        <thead><tr><th style={{ minWidth: 200 }}>Task</th><th>Status</th><th>Priority</th><th>Due date</th><th>Scheduled</th><th>Est. min</th><th>Goal</th><th></th></tr></thead>
        <tbody>
          {tasks.map(t => {
            const c = t.dueDate ? urgencyColor(t, dateStr) : "slate";
            const sched = t.taskType === "recurring" ? null : getTaskSchedule(t, allData);
            return (
              <tr key={t.id}>
                <td>
                  <EditableCell value={t.title} type="text" onChange={v => onUpdate({ ...t, title: v })} />
                  {t.taskType === "recurring" && <Tag color="indigo">recurring</Tag>}
                </td>
                ...
                <td>
                  {sched ? <button className="btn-ghost sched-link" onClick={() => onEditFull(t)} style={{ fontSize: 12, fontWeight: 700, color: "var(--indigo-text)" }}>{fmtDateShort(sched.date)}, {minutesToTime(sched.start)}</button>
                    : t.taskType === "recurring" ? <span style={{ fontSize: 12, color: "var(--text-tertiary)" }}>daily slot</span>
                    : <span style={{ fontSize: 12, color: "var(--coral-text)" }}>not scheduled</span>}
                </td>
...
function GoalCard({ g, tasks, habits, onUpdate, onDelete }) {
  ...
  return (
    <div className="card">
      ...
      {history.length > 0 && (
        <div className="goal-history">{history.slice().reverse().slice(0, 4).map(h => (
          <div key={h.id} className="goal-history-row"><span className="goal-history-time">{fmtDateTime(h.timestamp)}</span><span className="goal-history-note">{h.note}</span><span className="goal-history-pct">{h.progress}%</span></div>
        ))}</div>
      )}
    </div>
  );
  ...
  return (
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}><Tag>~{workout.estimatedMinutes} min</Tag><button className="btn-ghost" onClick={onExpand} title="Open full screen"><Icon name="expand" /></button></div>
      </div>
...
function PRTable({ exerciseHistory }) {
  ...
  return (
    <table className="monday-table">
      ...
      <tbody>{rows.map(r => (<tr key={r.key}><td style={{ fontWeight: 700 }}>{r.name}</td><td><Tag color="amber">{r.pr} lb</Tag></td><td>{fmtDateShort(r.prDate)}</td><td>{r.last} lb <span style={{ color: "var(--text-tertiary)", fontSize: 12 }}>({fmtDateShort(r.lastDate)})</span></td></tr>))}</tbody>
    </table>
  );
}
...
function FitnessTab({ state, onUpdate, onOpenGoalModal }) {
  ...
  const completeWorkout = (workoutIdx) => {
    ...
    onUpdate({ workoutLog: [...workoutLog, { id: uid(), date: todayStr(), dayLabel: `Day ${workoutIdx + 1}`, exercises: w.main, completedAt: new Date().toISOString() }], exerciseHistory: [...exerciseHistory, ...newHistory] });
    setExpandedIdx(null);
  };
  ...
  return (
    <div>
      ...
      {fitTab === "plan" && (
        <div>
          <p style={{ fontSize: 13.5, color: "var(--text-secondary)", lineHeight: 1.5, marginBottom: 14 }}>Each session targets ~60 minutes: warm-up, main work, cool-down — adapted to your "{goalLabel.toLowerCase()}" goal. Weight suggestions increase gradually based on what you last logged. Every field is editable.</p>
          <div className="workout-card-grid">{plan.map((w, i) => <WorkoutCard key={w.id} workout={w} dayLabel={`Day ${i + 1}`} onUpdateExercise={(exId, updated) => updateExercise(i, exId, updated)} onAddExercise={() => setShowAddExercise(i)} onComplete={() => completeWorkout(i)} onExpand={() => setExpandedIdx(i)} />)}</div>
      ...
      {fitTab === "calendar" && (
        <div>
          ...
          <div className="week-grid">{Array.from({ length: 7 }).map((_, i) => { const d = addDays(todayStr(), i); const completedThatDay = workoutLog.filter(w => w.date === d); const isWorkoutDay = i < settings.fitnessFrequency; return (
            <div key={d} className={"week-day-cell" + (i === 0 ? " is-today" : "")}>
              <p className="week-day-cell-label">{fmtDateShort(d)}</p>
              {completedThatDay.map(w => <div key={w.id} className="week-mini-chip chip-green">{w.dayLabel} done</div>)}
          ...
          {workoutLog.length > 0 && (
            <div style={{ marginTop: 20 }}>
              ...
              {workoutLog.slice().reverse().slice(0, 10).map(w => (<div key={w.id} className="block-row"><div className="block-stripe stripe-teal" /><div style={{ flex: 1 }}><p className="block-title">{w.dayLabel} — {fmtDate(w.date)}</p><p className="block-subtext">{w.exercises.length} exercises logged</p></div></div>))}
            </div>
          )}
        </div>
      )}
        ...
        <div>
          ...
          {sortedStats.length === 0 ? <p style={{ fontSize: 13.5, color: "var(--text-secondary)" }}>No entries yet — log your weight to start tracking trends.</p> : (
            <div>
              ...
              {sortedStats.slice().reverse().slice(0, 8).map(s => (<div key={s.id} className="block-row"><div className="block-stripe stripe-indigo" /><div style={{ flex: 1 }}><p className="block-title">{s.weight} lb{s.bodyFat ? ` · ${s.bodyFat}% body fat` : ""}</p><p className="block-subtext">{fmtDate(s.date)}{s.note ? ` — ${s.note}` : ""}</p></div></div>))}
            </div>
          )}
  ...
  return (
    <div>
      ...
      {habitStats.length === 0 ? <p style={{ fontSize: 13.5, color: "var(--text-secondary)" }}>No active habits yet.</p> : habitStats.map((h, i) => (
        <div key={i} className="block-row"><div className={"block-stripe " + (h.rate >= 70 ? "stripe-green" : h.rate >= 40 ? "stripe-amber" : "stripe-coral")} /><div style={{ flex: 1 }}><p className="block-title">{h.name}</p><div className="progress-track" style={{ marginTop: 4 }}><div className="progress-fill" style={{ width: `${h.rate}%` }} /></div></div><span style={{ fontSize: 13, fontWeight: 800, minWidth: 70, textAlign: "right" }}>{h.completed}/{h.applicable}</span></div>
      ))}
    </div>
  );
}
...
function MainApp({ session }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!error && row && row.data) setData({ ...emptyData(), ...row.data, settings: { ...DEFAULT_SETTINGS, ...(row.data.settings || {}) } });
  ...
  const saveTaskFromModal = (task) => { const exists = data.tasks.some(t => t.id === task.id); update({ tasks: exists ? data.tasks.map(t => t.id === task.id ? task : t) : [...data.tasks, task] }); setTaskModal(null); };
  const addTaskQuick = () => update({ tasks: [...data.tasks, { id: uid(), title: "New task", category: taskFilter, taskType: "one_time", dueDate: todayStr(), targetDate: "", estimatedMinutes: 30, priority: "medium", partOfDay: "midday", status: "not_started", notes: "", goalId: "" }] });

  const saveEvent = (event) => { const exists = data.events.some(e => e.id === event.id); update({ events: exists ? data.events.map(e => e.id === event.id ? event : e) : [...data.events, event] }); setEventModal(null); setBlockDetail(null); };
  ...
  const handleBlockDelete = (block) => {
    if (block.taskId) { if (block.recurring) { updateTask({ ...data.tasks.find(t => t.id === block.taskId), active: false }); } else { update({ blocks: { ...data.blocks, [viewDate]: (data.blocks[viewDate] || []).filter(b => b.taskId !== block.taskId) } }); } }
    else if (block.type === "event") deleteEvent(block.eventId || block.id);
  };
  const handleBlockEdit = (block) => {
    if (block.type === "event") setBlockDetail(block);
  ...
  const tasksForFilter = data.tasks.filter(t => t.category === taskFilter).sort((a, b) => (a.dueDate || "9999").localeCompare(b.dueDate || "9999"));
  ...
  return (
    <div className="app">
      ...
      <div className="main">
        ...
        {activeTab === "today" && (
          <div>
            ...
            <div className="stat-grid">
              ...
              <StatCard label="Overdue" value={data.tasks.filter(t => t.taskType !== "recurring" && t.status !== "done" && t.dueDate && t.dueDate < viewDate).length} color="amber" />
            </div>
            ...
            {blocks.length === 0 ? <div className="empty-state"><p>Nothing scheduled yet. Add tasks, habits, or events and they'll auto-fill your day without overlapping.</p></div> : <DayTimeline blocks={blocks} dateStr={viewDate} settings={data.settings} onEdit={handleBlockEdit} onStatusChange={markTaskDone} onDelete={handleBlockDelete} />}
            ...
            {habitsToday.length > 0 && (
              <div style={{ marginTop: 24 }}>
                ...
                {habitsToday.map(hh => { const done = data.completedLog.some(c => c.habitId === hh.id && c.date === viewDate); return (<label key={hh.id} className="habit-check"><input type="checkbox" checked={done} onChange={() => toggleHabitDone(hh.id)} /><span className={done ? "habit-done-text" : ""}>{hh.name} <span className="habit-time-tag">{hh.time}</span></span></label>); })}
              </div>
            )}
            <div style={{ marginTop: 24 }}>
        ...
        {activeTab === "tasks" && (
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 14 }}>
              <button className="btn btn-sm" onClick={() => setTaskModal({})}><Icon name="repeat" size={14} /> New (with options)</button>
              <button className="btn btn-primary btn-sm" onClick={addTaskQuick}><Icon name="plus" size={14} /> Quick add</button>
            </div>
            {tasksForFilter.length === 0 ? <div className="empty-state"><p>No {taskFilter} tasks yet.</p></div> : <TaskTable tasks={tasksForFilter} goals={data.goals} allData={data} onUpdate={updateTask} onDelete={deleteTask} onEditFull={setTaskModal} dateStr={viewDate} />}
          </div>
        )}
              ...
              <div>
                {data.habits.map(hh => (
                  <div key={hh.id} className="card" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px" }}>
                    <div style={{ flex: 1 }}><p style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>{hh.name}</p><div style={{ display: "flex", gap: 6, marginTop: 6 }}><Tag>{hh.frequency === "daily" ? "Every day" : "Weekly"}</Tag><Tag>{hh.duration}m</Tag><Tag color="indigo">{hh.time}</Tag>{!hh.active && <Tag color="coral">paused</Tag>}</div></div>
        ...
        {taskModal !== null && <TaskModal task={taskModal.id ? taskModal : null} defaultCategory={taskFilter} goals={data.goals} onSave={saveTaskFromModal} onClose={() => setTaskModal(null)} />}
        {blockDetail && <BlockDetailModal block={blockDetail} dateStr={viewDate} onClose={() => setBlockDetail(null)} onSaveEvent={saveEvent} onDeleteEvent={deleteEvent} />}
      </div>
